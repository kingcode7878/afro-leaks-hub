      ...
      <header className="p-4 flex justify-between items-center border-b border-zinc-800 sticky top-0 bg-black/80 backdrop-blur-md z-[60]">
        ...
        <div className="flex bg-zinc-900 rounded-lg p-1 relative z-[70]">
          <button 
            ...
            className={`p-1.5 rounded-md cursor-pointer ${viewMode === 'dashboard' ? 'bg-zinc-700 text-white' : 'text-zinc-500'}`}
          >
            <LayoutGrid size={18} />
          </button>
          <button 
            onClick={() => setViewMode('category')} 
            className={`p-1.5 rounded-md cursor-pointer ${viewMode === 'category' ? 'bg-zinc-700 text-white' : 'text-zinc-500'}`}
          >
            <List size={18} />
          </button>
        </div>
      ...
      <nav className="flex justify-around border-b border-zinc-800 bg-black sticky top-[61px] z-50 overflow-x-auto no-scrollbar">
        {tabs.map((tab) => (
          <button 
            ...
            className={`relative py-4 px-4 flex items-center gap-2 text-xs font-bold transition-colors z-[55] whitespace-nowrap ${category === tab.id ? 'text-white' : 'text-zinc-500'}`}
          >
            {tab.icon} {tab.id}
            {category === tab.id && (
              <motion.div 
